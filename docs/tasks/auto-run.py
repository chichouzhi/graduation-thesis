...
# -------------------------------------------------
# Single-task invariant: at most one `running`; no batch demotion
# -------------------------------------------------
def reconcile_single_running_task(data):
    """Enforce **at most one** task in ``running`` (no concurrent in-flight rows).

    - If **multiple** ``running`` → fatal (graph must be repaired manually).
    - If **exactly one** ``running``:
      - If ``.task.lock`` exists and is **not stale** → refuse to start (another live
        ``auto-run.py`` is likely waiting on INPUT with the lock held).
      - Else (no lock, or stale lock): demote **only that task** back to ``ready`` and
        remove a stale lock file. This recovers abandoned runs after Ctrl+C **without**
        reopening unrelated tasks.
    - If **zero** ``running``: remove **stale** lock only.

    Returns True if ``data['tasks']`` was mutated (caller should ``save_tasks``).
    """
    tasks = data["tasks"]
    ...
    if len(running) > 1:
        ...
        print(
            "ERROR: invariant violated — multiple tasks have status `running`: "
            + ", ".join(ids),
            file=sys.stderr,
        )
        print(
            "Fix docs/tasks/architecture-task-graph.json so at most one task is `running`, "
            "or set all but one back to `ready`/`done` manually.",
            file=sys.stderr,
        )
        sys.exit(1)
    if not running:
        if LOCK_FILE.exists() and _lock_is_stale():
            LOCK_FILE.unlink(missing_ok=True)
        return False
    sole = running[0]
    rid = sole["id"]
    if LOCK_FILE.exists():
        if not _lock_is_stale():
            raise RuntimeError(
                f"Cannot start Task OS: task {rid!r} is `running` and {LOCK_FILE} is held "
                "(not stale). Another `auto-run.py` may be waiting on INPUT — use that "
                "terminal, or stop it before starting a second one."
            )
        LOCK_FILE.unlink(missing_ok=True)
    sole["status"] = "ready"
    sole.pop("done_at", None)
    return True
...
# -------------------------------------------------
def main():
    print("Task OS v2.3 starting (single-task dequeue loop; no batch running recovery)...")
    ...
    while True:
        ...
        if reconcile_single_running_task(data):
            save_tasks(data)
        ...
        try:
            ...
            # Mark running before writing queue so queue.json matches graph
            # (next_task is the same dict as the entry in tasks).
            for t in tasks:
                if t["id"] == task_id:
                    t["status"] = "running"
            ...
            print(
                "\nInterrupted. Task left as `running` without advancing to `done`. "
                "Next `auto-run.py` start will demote **only that** orphan `running` "
                "back to `ready` (if lock is gone or stale), then dequeue again."
            )
            raise
        finally:
            release_lock()
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description=(
            "Task OS: strict single-task loop — dequeue one ready task into queue.json, "
            "mark it running, wait for ENTER, mark done; repeat until no eligible ready tasks. "
            "No concurrent runners; no batch demotion of all running tasks."
        ),
    )
    p.add_argument(
        "--validate-only",
